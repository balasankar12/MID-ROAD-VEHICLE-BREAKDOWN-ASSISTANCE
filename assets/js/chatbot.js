jQuery(document).ready(function($) {
	...
	$(document).on("submit", "#messenger", function(e) {
		e.preventDefault();
			$value = $("[name='msg']").val();
			$msg = '<div class="msg user"><span class="responsText">' + $value + '</span></div>';
			if($value){
			...
			// start ajax code
			$.ajax({
				url: 'chatbotmessage.php',
				type: 'POST',
				data: 'text='+$value,
				success: function(result){
					$replay = '<div class="msg"><span class="responsText">' + result + '</span></div>';
					$(".Messages_list").append($replay);
	...
	// 	// 	if (val.indexOf("vehicle") > -1 || val.indexOf("gf") > -1) {
